...
const vector<vector<vector<pair<int, int> > > >& MultiImages::getFeaturePairs() const {
	if (feature_pairs.empty()) {
		...
		for (int i = 0; i < images_match_graph_pair_list.size(); ++i) {
			...
			//保存特征点连接图像，并输出匹配特征点数量,定义在下方。
			//参数1：阶段名。参数2：参与匹配的图像名。参数3：得到的匹配结果，图片大小等参数。
			writeImageOfFeaturePairs("sRANSAC", match_pair, result);//match_pair:匹配图像名。
...
vector<pair<int, int> > MultiImages::getFeaturePairsBySequentialRANSAC(const pair<int, int>& _match_pair,
	...
	//单应性模型最小点
	//全局最大迭代 = log(1-opencv的默认置信度)/log(1-全局真概率的单应性模型最小点次幂))
	//默认置信度:进行多次选取后，至少有一次选取的n个点都是局内点的概率
	//全局真概率:全部数据中局内点的比例
	//单应性模型最小点:一次选取的n个点
	...
	Mat H(3, 3, CV_64F);
	//Opencv内置函数求全局单应性矩阵，CV_RANSAC为采用RANSAC方法。
