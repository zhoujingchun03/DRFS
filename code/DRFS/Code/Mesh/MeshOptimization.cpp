...
void MeshOptimization::prepareContentPreservingTerm(vector<Triplet<double> >& _triplets,
	...
	if (content_preserving_equation.second) {
		//Linear sampling point coordinates are converted to grid interpolation format.  线性采样点坐标转换为网格插值格式。
		...
		for (int i = 0; i < multi_images->images_data.size(); i++) {
			...
			for (int j = 0; j < content_interpolation[i].size(); j++) {
				...
				for (int k = 2; k < content_interpolation[i][j].size(); k++) { // Iterate over the sampling points in a line  遍历直线上的采样点
					sampleCount++;
					...
					for (int dim = 0; dim < DIMENSION_2D; ++dim) { //sample pointX and y
						for (int p = 0; p < multi_images->images_data[i].mesh_2d->getPolygonVerticesCount(); ++p) {// Iterate over 4 points of each grid
							// The sampling point pairs the weight values of the 4 points in the grid 取样点将网格中 4 个点的权重值配对起来
							_triplets.emplace_back(content_preserving_equation.first + eq_count + dim,

								// The location is the data block part of the 4 points in the grid where the sampling point is located  位置是取样点所在网格中 4 个点的数据块部分
							...
							if (dim == 0) { // x eq
								//The sampling point corresponds to the weight value of four points in the grid at the beginning point of the line采样点与直线起始点网格中四个点的权重值相对应
								_triplets.emplace_back(content_preserving_equation.first + eq_count + dim,

									//The position is the data block part of the four points of the grid where the starting point of the line segment is located 位置是线段起点所在网格四点的数据块部分
