...
Mat NISwGSP_Stitching::solve(const BLENDING_METHODS& _blend_method, vector<vector<Point2> >& original_vertices) {
	...
	prepareAlignmentTerm(triplets);//准备对准项
	prepareSimilarityTerm(triplets, b_vector);//准备相似项


	original_vertices = getImageVerticesBySolving(triplets, b_vector); //44444，通过求解获得图像顶点


	Size2 target_size = normalizeVertices(original_vertices);//归一化顶点


	Mat result = multi_images.textureMapping(original_vertices, target_size, _blend_method);//纹理映射
...
Mat NISwGSP_Stitching::solve_content(const BLENDING_METHODS& _blend_method, vector<vector<Point2> >& original_vertices) {
	...
	prepareContentPreservingTerm(triplets, b_vector);  //准备内容保护项
