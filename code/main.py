...
if __name__ == '__main__':
    ...
    #修改目录
    path_path = "C:\\Users\\grey\\Desktop\\DRFS\\01code\\"
    ...
    ##  Our源码
    ##
    source0 = path_path + "OutputImages\\0.jpg"
    target0 = path_path + "input-data\\Input\\0.jpg"
    copyfile(source0, target0)
    source1 = path_path + "OutputImages\\1.jpg"
    target1 = path_path + "input-data\\Input\\1.jpg"
    copyfile(source1, target1)

    print('********    Our-star   ********')
    folder = path_path
    path = folder + "/input-data"
    files = os.listdir(path)
    files = natsort.natsorted(files)
    for i in range(2, len(files)):
        ...
        if os.path.isdir(filepath):
            print('*****    dir   *****', file)
            cmd_str = 'cd ' + folder + ' && ' + 'DRFS.exe ' + file
            run_cmd_Popen_fileno(cmd_str)
    source00 = path_path + "input-data\\0_results\\Input-result\\Input-Ours_.png"
    target00 = path_path + "Output\\Our.png"
    copyfile(source00, target00)
    print('Our-done')
